...
module bed(bedplateX  = 325, bedplateY = 341, bedcornerrounding= 7.5)
{
...
module thermistor_channel() {
  ...
  // We want it to land in center of printable area. (It can't be, the ear is in the centre of the printable area! Actual distance is 19.2 from centre .. :Dan)
...
demo() {
//bed();
//bed(325,341,7.5); // standard bed
bed(325,351,7.5); // 10mm larger bed in Y than standard :)
//projection(cut = true)  translate([0, 0, -bed_thickness]) bed(325,341,7.5); // standard bed
//projection(cut = true)  translate([0, 0,-2]) bed(325,341,7.5); // standard bed
//projection(cut = true)  translate([0, 0,-6]) bed(325,341,7.5); // standard bed
}