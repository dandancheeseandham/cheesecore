...
module electronics_box(box_size_z,box_size_y, box_depth, acrylic_thickness) 
{
	...
	mirror_xz() 
	{
		...
		//electronics_box_corner(cornersize = 15, acrylicdepth = acrylic_thickness ,height = box_depth, ledgewidth = 10 , ledgethickness = 4); ;  //electronics box corners // standard corners
	...
	color(acrylic_color()) 
	{
			...
			mirror_xy() 
			{
				translate ([length/2-topscrewhole_x,box_depth/2-topscrewhole_y,0]) singlescrewhole(3,0.5);
			}
			if (stepper_cables) 
			{
			...
			if (IEC == true) 
			{		
		    translate ([length-IEC_cutout_distance-cut_out_width,15.25,-epsilon/2]) cube ([cut_out_width,cut_out_depth,acrylic_thickness+epsilon]);
...
module electronics_cover_panel(x, y, thickness, panelcornerrounding)
{
	...
	difference() 
	{
		...
		difference ()
        {
			...
			//16 vents
			if (laser_cut_vents == true) for(vents = [0 : 15]) translate ([PSU_hole_pos_x,PSU_hole_pos_z-(vents*(vent_height + gap_between_vents)),-20]) longscrewhole(vent_length,vent_height,0) ;
			if (laser_cut_vents == false) translate ([PSU_hole_pos_x,PSU_hole_pos_z-131,-20]) cube ([91,131,40]);    
			if (laser_cut_vents == true) 
			{
				translate ([-x/2+fan_pos_x,y/2+thickness-fan_pos_z,-acrylic_thickness])		fan_grill_difference(32,3.5,40,8 ,acrylic_thickness*3);
				translate ([-x/2+fan_pos_x,y/2+thickness-fan_pos_z-50,-acrylic_thickness]) 	fan_grill_difference(32,3.5,40,8 ,acrylic_thickness*3);
			}
